...
router.get('/edit-reservation/:id', async (req, res, next) => {
  try {
...
// handle reservation edit on form submit
router.post('/edit-reservation/:id', async (req, res, next) => {
  try {
    const reservation = await Reservation.get(req.params.id);
    ...
    return res.redirect(`/edit-reservation/${reservation.id}`);
  }
  catch (err) {
    return next(err);
  }
